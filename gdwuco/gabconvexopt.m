...
definput.flags.S0={'exact','approx'};
%definput.flags.S0={'approx','exact'};
definput.flags.timestep={'constant','dynamic'};
        ...
        try
            ...
            try
                ...
                try
                    ...
                    error('GABOPTDUAL: Cannot pinv the matrix...');
                end
            end
        end
...
% - small S0 norm -
    if kv.delta %frequency constraint
        ...
        if flags.do_exact
            ...
            [~,B]=gabframebounds(gauss,a,M);
            param_S0.A= @(x) dgt(x,gauss,a,M)/sqrt(B);
            param_S0.At= @(x) idgt(x,gauss,a,L)/sqrt(B);
            param_S0.tight = 0;
            param_S0.maxit = 10;
            param_S0.tol = 1e-2;          
        end
        ...
        if flags.do_debug
            param_S0.verbose=2; % display the results
        else
        ...
        if flags.do_exact
        g10.eval= @(x) kv.deltaw*norm(reshape(dgt(x,gauss,1,L),[],1),1); % objectiv function
        else
        g10.eval= @(x) kv.deltaw*norm(reshape(dgt(x,gauss,a,M),[],1),1); % objectiv function 
        end
        F{end+1} = g10;
    end
...
% - small weighted S0 norm -
    if kv.deltaw %frequency constraint
        ...
        if flags.do_exact
            ...
            a2 = a;
            M2 = M;
            [~,B]=gabframebounds(gauss,a2,M2);
            param_S0.A= @(x) dgt(x,gauss,a2,M2)/sqrt(B);
            param_S0.At= @(x) idgt(x,gauss,a2,L)/sqrt(B);
            param_S0.tight = 0;
            param_S0.maxit = 10;
            param_S0.tol = 1e-2;          
        end
        ...
        if flags.do_exact
            ...
            if mod(L,2*a)
                tmp = mod(L,2*a);
                w1=[0:a:(L-tmp)/2,(L-tmp)/2:-a:1]'; 
            else         
                w1=[0:a:L/2-1,L/2:-a:1]';
            end
            if mod(L,2*L/M)
                tmp = mod(L,2*L/M);
                w2=[0:(L/M):(L-tmp)/2,(L-tmp)/2:(-L/M):1]'; 
            else         
                w2=[0:(L/M):L/2-1,L/2:(-L/M):1]';
            end
            w1=w1/sqrt(L);
            w2=w2/sqrt(L);
            W = repmat(w2,1,L/a).^2+repmat(w1',M,1).^2;
        end
        ...
        if flags.do_exact
        g15.eval= @(x) kv.deltaw*norm(reshape(W.*dgt(x,gauss,1,L),[],1),1); % objectiv function
        else
        g15.eval= @(x) kv.deltaw*norm(reshape(W.*dgt(x,gauss,a,M),[],1),1); % objectiv function 
        end
        F{end+1} = g15;
        
    end
 
% - small weighted S2 norm -
    if kv.deltaw2 %frequency constraint
        gauss=pgauss(L,1);
        ...
        if flags.do_exact
            [A,B]=gabframebounds(gauss,1,L);
            AB=(A+B)/2;
            param_S2.A= @(x) dgt(x,gauss,1,L)/sqrt(AB);
            param_S2.At= @(x) idgt(x,gauss,1,L)/sqrt(AB);
            param_S2.tight = 0;
            param_S2.maxit = 5;
            param_S2.tol = 1e-2;
        else
            [~,B]=gabframebounds(gauss,a,M);
            param_S2.A= @(x) dgt(x,gauss,a,M)/sqrt(B);
            param_S2.At= @(x) idgt(x,gauss,a,L)/sqrt(B);
            param_S2.tight = 0;
            param_S2.maxit = 10;
            param_S2.tol = 1e-2;          
        end
        ...
        if flags.do_debug
            param_S2.verbose=2; % display the results
        else
            param_S2.verbose=0; % Do not display anything
        end
        ...
        param_S2.weights=W;
        nb_priors=nb_priors+1;
        g16.prox= @(x,T) prox_l2(x,T*kv.deltaw2,param_S2);   
        if flags.do_exact
        g16.eval= @(x) kv.deltaw2*norm(reshape(W.*dgt(x,gauss,1,L),[],1),2); 
        else
        g16.eval= @(x) kv.deltaw2*norm(reshape(W.*dgt(x,gauss,a,M),[],1),2); 
        end
        F{end+1} = g16;
...
% parameter for the solver
    ...
    if flags.do_dynamic
        param_solver.gamma = 10/log(L);
        nit = min(50,ceil(3/4*kv.maxit));
        param_solver.do_ts=@(x) log_decreasing_ts(x, ...
            10/log(L), 0.1/log(L), nit); 
    else
        param_solver.gamma = 1/log(L);
    end
    if flags.do_quiet
