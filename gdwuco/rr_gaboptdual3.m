%RR_GABOPTDUAL3 Optimization of dual gabor windows 3
%                           
...
%   Itersine analysis window with $L_g=120$ $L=240$, $a=15$ and $M=120$,
...
%   $31$ values above $-80$~dB (relative to the maximum amplitude) on an
%   interval of length $101$, approximatively the number of WR equations 
%   $L a/M=30$. In this configuration, we observe a spread of the values.
%   
%
...
% %% Convex optimization mixed case
% 
% %parameter for convex optimization
% mu=50;      % smoothing parameter
% gamma=10; % weight for the concentration in time
% drawnow;
% % solving the problem
% fprintf(' Solve the convex optimization problem: time and frequency concentration\n');
% gd=gaboptdual(g,a,M,'mu',mu,'gamma',gamma,...
%     'maxit',maxit,'tol',tol,'evolution',1,'dynamic');
% gd=real(gd);
% 
% % Verification: this number should be close to 0.
% fprintf('  Reconstruction error of optimization problem: %g \n',...
...
%% Effect of different windows on synthesis

% %signal
% % f=[ sin(2*pi*(1:L/4)/(L/32)),...
% %     sin(2*pi*(1:L*3/8)/(L/8)),sin(2*pi*((1:L*3/8)/(L/10)).^2)]';
% 
% f=zeros(1,L);
% 
% % figure(23);
...
% figure(28);
% cm=c;
% cm(M/4+[1:M/a],L/a/2)=max(max(abs(c(:))),1);
% plotdgtreal(cm,a,M,1,50,'clim',clim);
% title('Original coefficients');
% save_name='modified_spectrogram';
% plotfig(save_name,paramplot );
% 
% g2=firwin('nuttall',60);
% a2=1;
% M2=120;
% 
% c_can=dgtreal(idgtreal(cm,gcan,a,M,L),g2,a2,M2);
% c_time=dgtreal(idgtreal(cm,gd,a,M,L),g2,a2,M2);
% 
% figure(29);
% plotdgtreal(c_can,a2,M2,1,50,'clim',clim);
% title('Synthesis');
% save_name='new_spectrogram_gcan';
% plotfig(save_name,paramplot );
% figure(30);
% plotdgtreal(c_time,a2,M2,1,50,'clim',clim);
% title('Alternative synthesis');
% save_name='new_spectrogram_gtime';
% plotfig(save_name,paramplot );
