%RR_GABFIRDUAL_SMOOTHNESS2 Optimization of dual gabor windows with support constraint
%                           
...
%   ratio of $1/5$ (see figure below, with $L_g = 240$, $a = 50$ and
...
if baw
    paramplot.pathfigure = 'figures/gabfirdual_smooth_tukeywin_02/'; 
else
    paramplot.pathfigure = 'figures_color/gabfirdual_smooth_tukeywin_02/'; 
end
...
[crit_mat(5,:),crit_mat2(5,:)] = compute_criteria(gd_S0,Lcomp,fac);
[crit_mat(6,:),crit_mat2(6,:)] = compute_criteria(gd_S0w,Lcomp,fac);
Y = (crit_mat(1:6,:) - repmat(min(crit_mat(1:6,:)),6,1)); 
Y = Y./repmat(max(Y(1:6,:)),6,1);

crit_mat = crit_mat(:,[2,3,8,9,10,11,4,5,1,6,7]);
XX = [10000,1000,10,100,100,1000,1,1,100,10,10]'*[1,1,1,1,1,1];
crit = crit_mat.*XX';

XX2 = [100,100,1,1,1]'*[1,1,1,1,1,1];
crit2 = crit_mat2.*XX2';
crit2 = crit2([1,5,2,3,4],:);
