...
%     computation. However, sometimes, it can not be computed.
%
...
if ~isfield(param, 'AAtinv'), param.AAtinv = pinv(param.A*param.A'); 
warning('You should pinv A At before...')
end
if ~isfield(param, 'verbose'), param.verbose = 1; end


% Projection  
% if isfield(param, 'AAtinv')
...
%     warning('off', MSGID)
%     AAt = param.A*param.A';
%     lambda = AAt \ (param.A*x-param.y);
%     sol = x - param.A'*lambda;
%     warning('on', MSGID)
% end
